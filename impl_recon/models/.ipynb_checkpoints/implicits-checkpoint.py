...
class OccupancyPredictor(torch.nn.Module):
    def __init__(self, total_latent_dim: int, spatial_dim: int, num_layers: int,
        ...
        in_channels = [total_latent_dim] * num_layers
        channels_with_coords = total_latent_dim + spatial_dim
        for lyr_id in self.layers_with_coords:
            in_channels[lyr_id] = channels_with_coords
        #print("total_latent_dim", total_latent_dim)
        self.res_layers = torch.nn.ModuleList(
            [block(in_channels[i], total_latent_dim) for i in range(num_layers - 1)])
...
class AutoDecoder(torch.nn.Module):
    ...
    def __init__(self, lat_dim: int, spatial_dim: int, image_size: torch.Tensor,
                 occnet_num_layers: int, occnet_layers_with_coords: List[int], additional_vector_size=0):
        """
        ...
        total_latent_dim = lat_dim + additional_vector_size
        self.occp_pred: torch.nn.Module
        self.occp_pred = OccupancyPredictor(total_latent_dim, spatial_dim, occnet_num_layers,
