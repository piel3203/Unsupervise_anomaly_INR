...
def load_volumes(volumes_dir: Path, casenames: List[str], spacing_scale: float = 1.0) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    ...
    for casename in casenames:
       ...
       # Get the spacing from the diagonal of the affine matrix
        spacing = np.diagonal(affine)[:3].copy()
        print("Initial spacing:",spacing)
        
        # Scale the spacing (increasing spacing reduces resolution)
        new_spacing = spacing * spacing_scale
        print("New spacing:",new_spacing)
        
        # Compute the zoom factor for each axis
        zoom_factors = spacing / new_spacing
        
        # Resample the volume to the new spacing
        resampled_volume = zoom(volume, zoom_factors, order=1)  # order=1 means linear interpolation
        print("RESAMPLED VOL SHAPE:", np.shape(resampled_volume))
        volumes.append(resampled_volume)
    ...
    volumes_tnsr = [torch.from_numpy(x).to(torch.float16) for x in volumes]
    spacings_tnsr = [torch.from_numpy(x).to(torch.float16) for x in spacings]
...
def generate_mid_slice_ids(label: torch.Tensor) -> torch.Tensor:
    ...
    # Find the indices of the foreground voxels
    ...
    # Calculate foreground positions
    ...
    if torch.sum(label) == 0:
        print("No foreground voxels found in the label.")
        center = torch.tensor(label.shape) // 2  # Set center to geometric center
        print("Setting center to geometric center:", center)
    else:
        # Calculate the center of the foreground
        center = torch.sum(fg_positions, dim=0) / torch.sum(label)
        center = torch.floor(center).to(torch.int64)  # Ensure valid indices
        print("Calculated Center:", center)

    # Ensure center values are valid indices
    for i in range(3):
        if center[i] < 0 or center[i] >= label.shape[i]:
            print(f"Center index {i} is out of bounds: {center[i]} (label shape: {label.shape})")
            center[i] = max(0, min(center[i], label.shape[i] - 1))  # Clamp to valid range
